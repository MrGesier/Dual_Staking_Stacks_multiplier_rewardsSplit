...
# ---------------------------- Utilities & formulas ---------------------------
def r_stx(phi, T, S0, m2, S2):
    return (phi * T) / (S0 + m2 * S2)
...
def apr_tier2(phi, T, S0, B1, m2, B2, S2, rho):
    """Tier-2 APR (%) – BTC + STX collateral."""
    reward  = (m2 * S2) * r_stx(phi, T, S0, m2, S2) + (m2 * B2) * r_btc(phi, T, B1, m2, B2)
...
m2  = st.sidebar.slider(
    ...
    help="Boost factor applied to Tier-2 weights on both legs."
)
...
with colA.expander(" ?  What is this?"):
    st.write("APR on Tier-1 (BTC-only) at the selected **B₂**; "
             "independent of ρ. Formula:  \n"
...
with colB.expander(" ?  What is this?"):
    st.write("APR on Tier-2 (BTC + STX). Formula:  \n"
             r"$$\mathrm{APR}_{\max}=\frac{m_2 S_2\frac{\varphi T}{S_0+m_2 S_2}+m_2 B_2\frac{(1-\varphi)T}{B_1+m_2 B_2}}{B_2+\frac{S_2}{\rho}}\times 100.$$")

colC.metric("Capacity:  B₂^max (Base ≥ target)", f"{B2_max_cap:,.0f} BTC")
...
# ----------------------------- Limit functions -------------------------------
st.markdown("### Cost-opportunity limit functions")
...
\frac{m_2 S_2\frac{\varphi T}{S_0+m_2 S_2}+m_2 B_2\frac{(1-\varphi)T}{B_1+m_2 B_2}}{B_2+\frac{S_2}{\rho}}\times 100
...
# For S2 in this map, keep the same policy as the point (α × minimum or manual S2 scaled to B2_point)
if policy.startswith("α"):
    S2_for_map = alpha * collat_min * B2_point * rho
else:
    S2_for_map = S2_point  # treat as given tokens for the chosen B2_point
...
# NEW: Validity zone (hatched): APR₁ ≥ target AND APR₂ ≥ target
valid1 = (APR_base_map >= base_target) & (APR2_map >= t2_target)
...
patch1 = mpatches.Patch(facecolor='none', edgecolor='grey', hatch='////',
                        label='Valid zone (APR₁ & APR₂ ≥ targets)')
...
# NEW: Validity zone (hatched) = collateral OK & APR₂ ≥ target & APR₁ ≥ target
valid2 = ok & (APR2 >= t2_target) & (BASE >= base_target)
ax2.contourf(B2G, S2G/1e6, valid2.astype(float),
             levels=[0.5, 1.1], colors='none', hatches=['////'], alpha=0)

# Legend hatch patch
patch2 = mpatches.Patch(facecolor='none', edgecolor='grey', hatch='////',
                        label='Valid zone (collateral & APR targets)')
...
# NEW: Validity zone (hatched): APR₂ ≥ target AND APR₁ ≥ target  (collateral auto-satisfied here)
valid3 = (APR2B >= t2_target) & (BASEB >= base_target)
...
st.markdown("---")
st.subheader("4) Tier-2 APR vs absolute ρ and B₂  —  APR₂≥target hatched")

rho_vals = np.linspace(50_000.0, 650_000.0, 321)  # same axes as vos figures
