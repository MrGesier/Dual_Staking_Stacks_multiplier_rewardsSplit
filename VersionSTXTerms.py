# app_churn.py — Dual Stacking Explorer (full app + churn dynamics + cleaned sidebar)
...
with st.sidebar.expander("Rewards model (optional)", expanded=False):
    st.markdown(
        ...
        "where ρ is your current **STX per BTC** (same input below)."
    )
...
# Ensure rho_norm_max exists even if expander collapsed (we created it above)
rho_scaled = np.clip(rho_clamped / max(1e-9 if 'rho_norm_max' not in locals() else rho_norm_max, 1e-9), 0.0, 10.0)
a_val = 0.0 if 'a_param' not in locals() else a_param
b_val = -1.0 if 'b_param' not in locals() else b_param
T_modeled_raw = 26.0 * a_val * np.exp(b_val * rho_scaled)
T_modeled = float(np.maximum(T_modeled_raw, 0.0))  # prevent negative rewards for plotting stability
...
# Churn acts on the 'point' configuration (B1, B2_point, S2_point) and yields effective B1/B2
B1_eff, B2_eff, S2_eff_point = float(B1), float(B2_point), float(S2_point)
churn_net_flow = 0.0  # +BTC moved from B1→B2 (negative means B2→B1)

if enable_churn:
    for _ in range(n_steps):
        # APRs using current effective state (APR_pure not needed for flow decision)
...
APR_t2_btc       = apr_tier2_btc_only(phi, T_eff, B1_eff, m2, B2_eff)  # BTC-only on Tier-2
...
# Grid φ vs m2 (uses post-churn B1*, B2*)
...
phi_base_line = 1.0 - (base_target/100.0)*(B1_eff + m2_grid * B2_eff)/T_eff
ax1.plot(m2_grid, phi_base_line, ls="--", color="cyan", label=f"APR₁ = {base_target:.1f}%")

phi_t2_line = []
for m2v in m2_grid:
    vals = np.array([apr_tier2_composite(ph, T_eff, S0, B1_eff, m2v, B2_eff, S2_for_map_eff, rho, exclude_s2_toggle) for ph in phi_grid])
...
BASE = ((1.0 - phi) * T_eff / (B1_eff + m2 * B2G)) * 100.0   # use post-churn B1*
PURE = apr_pure(phi, T_eff, S0, S2G_eff, rho)

for i in range(APR2.shape[0]):
    for j in range(APR2.shape[1]):
        if ok[i, j]:
            APR2[i, j] = apr_tier2_composite(phi, T_eff, S0, B1_eff, m2, B2G[i, j], S2G_eff[i, j], rho, exclude_s2_toggle)
...
for i in range(APR2B.shape[0]):
    for j in range(APR2B.shape[1]):
        APR2B[i, j] = apr_tier2_composite(phi, TT[i, j], S0, B1_eff, m2, B2GG[i, j], S2GG_eff[i, j], rho, exclude_s2_toggle)
        BASEB[i, j] = apr_base(phi, TT[i, j], B1_eff, m2, B2GG[i, j])
...
ax3.set_title(
    ...
    f"α={alpha:.2f}× min collateral | B₁*={B1_eff:.0f} | m₂={m2:.1f}"
)
st.pyplot(fig3)
...
for i in range(APR2H.shape[0]):
    for j in range(APR2H.shape[1]):
        APR2H[i, j] = apr_tier2_composite(phi, T_eff, S0, B1_eff, m2, B2H[i, j], S2H_eff[i, j], RHO[i, j], exclude_s2_toggle)
...
ax4.set_title(f"Tier-2 APR vs ρ and B₂  (α={alpha:.2f}× min collateral, φ={phi:.2f}, m₂={m2:.1f}, B₁*={B1_eff:.0f})")
st.pyplot(fig4)
...
B2_tot_true = B2_eff + user_B2 if recompute_pool else B2_eff
S2_tot_true = S2_eff_point + user_S2 if recompute_pool else S2_eff_point

# Rates for rewards (use T_eff consistently)
rs_u = r_stx(phi, T_eff, S0, S2_tot_true)           # BTC per STX (STX-leg, unboosted)
rb_u = r_btc(phi, T_eff, B1_eff, m2, B2_tot_true)   # BTC per BTC (BTC-leg)
...
with st.expander(" ?  How are these computed?"):
    st.write(
        ...
        r"$\mathrm{APR}^{(\mathrm{BTC})}_2=m_2\frac{(1-\varphi)T_{\text{eff}}}{B_1^{\ast}+m_2(B_2^{\mathrm{tot}})}\times100$,"
        " applied to your BTC.  \n"
...
# ---------------------------------------------------------------------------
# DPI controller at the END (affects all figures on next redraw)
# ---------------------------------------------------------------------------
st.markdown("---")
